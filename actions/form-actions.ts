...
export async function generateForm(id: string, description: string) {
  ...
  const promptExplanation =
    "Based on the description, generate a survey object with 3 fields:  id a random number between 1000 and 9999(string) for the form, type: chosen from SelectField, DateField, TextField, NumberField, TextAreaField, CheckboxField, extraAttributes an object with label(string), helperText(string, required(boolean), placeholder:(string), options(array of string). if you choose a checkboxField the question need to be answered only with true or folse. return the response as an array of object, return only the array no other comments. Do not leave any space or use any special character. Important You answer need to be parsed as json object";
